...
$response = new ChooseCourseResponse();
...
if ($check_choose_result->num_rows != 0) {
    $response->format($response->EXIST);
    return_data($response);
}
...
$sql = "insert into tb_choose(student_id, course_id, test_name, test_score) values('$user->linkID','$courseID','',0)";
